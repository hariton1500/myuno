...
class GameServer {
  ...
  void answerTo(List<Socket> to, Map<String, String> msg) {
  ...
  handleMsg(message, Socket _socket) {
    ...
    switch (msg['type']) {
      ...
      case 'createGame':
        ...
        gamesList.add(_gameName);
        out(gamesList, 'Games List');
        playersInGames[msg['name']] = msg['name'];
        out(playersInGames, 'Players in Games');
        //временно сразу добавляем бота
        String _botName = 'Bot' + DateTime.now().hour.toString() + DateTime.now().minute.toString();
        players.add(_botName);
        playerIsHuman[_botName] = false;
        playersInGames[_botName] = _gameName;
        answerTo([clientsSockets[_gameName]], {'type' : 'playersInGamesUpdate', 'newPlayerInGame' : jsonEncode(playersInGames.keys.where((__name) => playersInGames[__name] == _gameName).toList())});
        break;
      case 'deleteGame':
        String _gameName = msg['name'];
      ...
      case 'addBot':
        ...
        String _botName = 'Bot' + DateTime.now().toLocal().toString();
        players.add(_botName);
        playerIsHuman[_botName] = false;
        playersInGames[_botName] = _gameName;
        out(playersInGames, 'Players in Games');
  ...
  onInGameAnswer(dynamic message) {
    ...
    if (_index != -1) {
      ...
      print('Game: ${_game.name}; Players: ${_game.humanPlayers}; Human players sockets:');
      _game.humanPlayers.forEach((_name) {
        if (playerIsHuman[_name]) print('$_name: ${clientsSockets[_name].address.toString()}:${clientsSockets[_name].port.toString()}');
      });
