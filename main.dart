...
class GameServer {
  ...
  void answerTo(List<String> to, Map<String, String> msg) {
    ...
    if (msg['type'] == 'inGame') if (msg['typeMove'] == 'setMast' || msg['typeMove'] == 'moverIs') lastMsg = msg;
    to.forEach((_player) {
      if (playerIsHuman[_player]) {
        ...
        if (msg.containsKey('typeMove')) {
          ...
          {
            Uno _game = games.firstWhere((__game) {return (__game.name == playersInGames[_player]);}, orElse: () => null);
      ...
      if (_aiMove.myMove != null) {
        ...
        _aiMove.myMove.forEach((_card) {
          dynamic _msg = '';
            ...
            _msg = {'type' : 'inGame', 'gameType' : 'addHeap', 'heap' : _card, 'name' : _player, 'gameName' : _game.name};
            onInGameAnswer(_msg);
        });
        dynamic _msg = {'type' : 'inGame', 'gameType' : 'playerMove', 'gameName' : _game.name, 'move' : json.encode(_aiMove.myMove), 'mast' : _aiMove.orderedMast != 0 ? _aiMove.orderedMast : null};
        print('sending: $_msg');
        onInGameAnswer(_msg);
      } else print('move: null');
      if (_aiMove.orderedMast != null) {
        print('ordered mast: ${_aiMove.orderedMast}');
      } else print('ordered mast: null');
      if (_aiMove.needCard != null) onInGameAnswer({'type' : 'inGame', 'gameType' : 'takeCardFromBase', 'name' : _player, 'gameName' : _game.name});
    ...
    switch (msg['type']) {
      case 'addPlayer':
        if (!players.contains(msg['name'])) {
          ...
          answerTo([msg['name']], {'type' : 'answer', 'result' : 'ok', 'mess' : 'Регистрация пройдена'});
        }
        else {
          answerTo([msg['name']], {'type' : 'answer', 'result' : 'notOk', 'mess' : 'Это имя уже занято'});
        }
        break;
      case 'getGamesList':
        String _fromName = msg['name'];
        answerTo([_fromName], {'type' : 'gamesListUpdate', 'gamesList' : jsonEncode(rooms)});
        break;
      case 'createGame':
      ...
      case 'deleteGame':
        ...
        List<String> _playersOfGame = playersInGames.keys.where((_name) => playersInGames[_name] == _gameName && playerIsHuman[_name]).toList();
        ...
        _playersOfGame.forEach((_player) => answerTo([_player], {'type' : 'gameDestroyed', 'name' : msg['name'], 'type2' : 'gamesListUpdate', 'gamesList' : jsonEncode(rooms)}));
        games.removeWhere((_game) => _game.name == _gameName);
        break;
      case 'enterGame':
        String _gameName = msg['gameName'];
        String _playerName = msg['who'];
        List<String> _playersInRoom = playersInGames.keys.where((__name) => playersInGames[__name] == _gameName).toList();
        ...
        if (_playersInRoom.isNotEmpty) {
          ...
          answerTo([msg['who']], {'type' : 'playersInGames', 'players' : json.encode(_playersInRoom), 'scores' : json.encode(_scores)});
        }
        ...
        answerTo(_playersInRoom, {'type' : 'playersInGamesUpdate', 'playerName' : _playerName, 'score' : '0'});
        break;
      case 'continueGame':
        String _roomName = msg['gameName'];
        List<String> _players = playersInGames.keys.where((__name) => playersInGames[__name] == _roomName).toList();
        ...
        answerTo([msg['who']], {'type' : 'playersInGames', 'players' : json.encode(_players), 'scores' : json.encode(_scores)});
        break;
      case 'addBot':
        String _gameName = msg['gameName'];
        //генерируем нового игрока-бота
        String _botName = 'Bot' + DateTime.now().hour.toString() + DateTime.now().minute.toString() + DateTime.now().second.toString();
        players.add(_botName);
        playerIsHuman[_botName] = false;
        playersInGames[_botName] = _gameName;
        scores[_botName] = 0;
        List<String> _toPlayers = playersInGames.keys.where((__name) => playersInGames[__name] == _gameName && playerIsHuman[__name]).toList();
        ...
        answerTo(_toPlayers, {'type' : 'playersInGamesUpdate', 'playerName' : _botName, 'score' : '0'});
        break;
      case 'leaveGame':
        String _gameName = msg['gameName'];
  ...
  onInGameAnswer(dynamic message) {
    ...
    int _index = games.indexWhere((game){return game.name == message['gameName'].toString();});
    if (_index != -1) {
      ...
      switch (message['gameType']) {
        ...
        case 'playerMove':
          ...
          _move.forEach((_card) {
            ...
            _game.cards[_game.humanPlayers[_game.currentMovePlayer]].remove(_card);  
          });
          ...
          if (toDo['setMast']) {
            ...
            if (_game.orderedMast == null) _game.orderedMast = _game.mastOf(__move.last);
            print('Ordered mast is: ${_game.orderedMast}');
            if (!checkForAWinner(_game)) answerTo([_game.humanPlayers[_game.currentMovePlayer]], {'type' : 'inGame', 'typeMove' : 'setMast', 'mast' : _game.orderedMast});
          } else _game.orderedMast = '';
          if (toDo['simpleCard']) {
            if (!checkForAWinner(_game)) answerTo(_game.humanPlayers, {'type' : 'inGame', 'typeMove' : 'moverIs', 'movePlayer' : _game.humanPlayers[_game.currentMovePlayer]});
          }
          if (toDo['moveTo']) {
            print('Sending to ${_game.humanPlayers} who is move now');
            if (!_checked) {
              if (!checkForAWinner(_game)) answerTo(_game.humanPlayers, {'type' : 'inGame', 'typeMove' : 'moverIs', 'movePlayer' : _game.humanPlayers[_game.currentMovePlayer]});
            } else answerTo(_game.humanPlayers, {'type' : 'inGame', 'typeMove' : 'moverIs', 'movePlayer' : _game.humanPlayers[_game.currentMovePlayer]});
          }
        ...
        case 'whatNextFirst?':
          ...
          if (_player == _currentPlayer) {
            if (_game.playerCanAddCardsToMove()) {
              answerTo([message['name']], {'type' : 'inGame', 'typeMove' : 'youCanAddCards', 'dost' : games.last.dostOf(games.last.cards['heap'].first)});
            } else {
        ...
        //отправляем игроку его карты
          answerTo([message['name']], {'type' : 'yourCards', 'cards' : json.encode(games[_index].cards[message['name']])});
        break;
        case 'getInitMove':
          //сообщаем про первую карту в куче
          answerTo([message['name']], {'type' : 'initMove', 'heap' : games[_index].cards['heap'].first});
        break;
        case 'addedCards':
  ...
  bool checkForAWinner(Uno _game) {
    ...
    if (_winner != '') {
      ...
      answerTo(_game.humanPlayers, {'type' : 'inGame', 'typeMove' : 'winner', 'winnerName' : _winner, 'scoreMap' : json.encode(_game.scores)});
  ...
  startGame(String gameName) {
    ...
    answerTo(playersOfGame, {'type' : 'runGame', 'gameName' : gameName, 'gameRunner' : gameName});
  }
}
